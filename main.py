...
def showFrame(imgplot, histplot, frameData, DispFrame, figure, imgsave):
...
def storeImage(nparray, figure):
    ...
    currDir = os.getcwd()
    folder= os.path.join(currDir,'Bucket' + str(figure)+'_Exp'+exposure +'_Patt'+numMasks+'_'+numMaskChanges+'_'+numSubPer+'_'+frame)
                ...
                if valid:
                    ...
                    while(self.DispImage.isChecked()):
                        ...
                        if stuckCt==50:
                            ...
                            dev.UpdateWireIns()
                            dev.ActivateTriggerIn(0x53,0x01)
                        elif full or oneFrameReady: #couldn't figure out difference between these
                            stuckCt = 1
    ...
    def rec_video(self):
        if (self.RecVideo.isChecked()):
            if str(bitfile)=="":
                ...
                vidfile = os.path.join(folder,filename1)
                while Path(vidfile).exists(): #check if video by the same name exists, if so, increase counter
                    i+=1
                    filename1 = 'Arrangement1_'+str(i)+'.avi'
                    vidfile = os.path.join(folder,filename1)
                filename2 
                video1 = cv2.VideoWriter(filename, -1, 1, (184,160))
                video2 = cv2.VideoWriter(filename, -1, 1, (184,160))
        else:
            #stop video
            vidRec = 0
            cv2.destroyAllWindows()
